...
const addFriend = async (req, res) => {
  try {
    const [user, friend] = await getUsers([req.params.username, req.params.friendUsername]);
    if (!user || !friend) return res.status(404).json(errorResponse("USER_NOT_FOUND"));
    if (user.friends.includes(friend._id)) return res.status(400).json(errorResponse("ALREADY_FRIENDS"));
